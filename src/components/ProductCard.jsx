...
const ProductCard = ({ id, image, brand, name, price, discountPercent, discountedPrice, sizes }) => {
        ...
        <div className="group relative flex flex-col h-full">
            <Link to={`/Product/${id}`} className="flex-grow">
                <div className="relative w-full pb-[100%] overflow-hidden rounded-lg shadow-md transition-all duration-300 hover:shadow-xl">
                    <img
                        className="absolute top-0 left-0 w-full h-full object-cover object-top transition-transform duration-300 group-hover:scale-110"
                        src={image}
                        alt={name}
                        loading="lazy"
                        onError={(e) => {
